...
const AdminApplicationsScreen = ({ navigation }) => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      const response = await fetch(
        `${SERVER_URL}/admin/application/${selectedApplication.id}/status`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${userToken}`,
  ...
  const handleDeleteApplication = async (applicationId) => {
    try {
      ...
      Alert.alert("신청 삭제", "이 신청을 삭제하시겠습니까?", [
        {
          text: "취소",
          style: "cancel",
        },
        {
          ...
          onPress: async () => {
            const response = await fetch(
              `${SERVER_URL}/admin/application/${applicationId}`,
              {
                method: "DELETE",
                headers: {
                  Authorization: `Bearer ${userToken}`,
      ...
      if (downloadResult.status === 200) {
        ...
        if (isAvailable) {
          try {
            ...
            Alert.alert(
              "파일 열기 실패",
              "파일을 열 수 없습니다. 파일은 다음 위치에 저장되었습니다: " +
                downloadResult.uri
            );
          }
        } else {
          Alert.alert(
            "알림",
            "파일이 다운로드되었지만, 이 기기에서는 파일을 열 수 없습니다. 파일은 다음 위치에 저장되었습니다: " +
              downloadResult.uri
          );
        }
  ...
  const confirmDeleteType = async () => {
    try {
      ...
      const response = await fetch(
        `${SERVER_URL}/admin/delete_application_type`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    return (
      <View style={styles.userSection}>
        <View style={styles.userHeader}>
          <View style={styles.userInfo}>
            ...
            <View style={styles.userDetails}>
              ...
              <Text style={styles.userStats}>ID: {userData.username}</Text>
              <Text style={styles.userStats}>
                총{" "}
                {Object.values(userData.applications).reduce(
                  (sum, app) =>
                    sum + (Array.isArray(app.files) ? app.files.length : 0),
                  0
                )}
                개의 파일
              </Text>
        ...
        {types.map((type) => {
          ...
          const app =
            userData.applications[type] || userData.applications[appType];
          if (!app) return null;
          ...
          return (
            <View key={`${userName}-${type}`} style={styles.typeSection}>
                ...
                <View style={styles.typeActions}>
                  <TouchableOpacity
                    style={[
                      styles.statusButton,
                      {
                        backgroundColor: STATUS_COLORS[app.status] || "#718096",
                      },
                    ]}
              ...
              <View style={styles.applicationInfo}>
                ...
                {app.notes && (
                  <Text style={styles.notes}>메모: {app.notes}</Text>
                )}
              </View>
                      ...
                      >
                        ...
                        >
                          ...
                          <View style={styles.fileDetails}>
                            ...
                            <Text style={styles.fileType}>{file.doc_id}</Text>
                          </View>
                        </TouchableOpacity>
                        <TouchableOpacity
        ...
        <View style={styles.modalContent}>
          ...
          <View style={styles.modalBody}>
            ...
            <Text style={styles.modalText}>
              {selectedUserToDelete}님의{" "}
              {APPLICATION_TYPES[selectedTypeToDelete]} 신청 내역을 모두
              삭제하시겠습니까?
            </Text>
