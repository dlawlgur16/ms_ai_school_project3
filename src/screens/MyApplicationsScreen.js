...
const MyApplicationsScreen = ({ navigation }) => {
    ...
    <View style={styles.applicationItem}>
      <View style={styles.applicationHeader}>
        <View style={styles.applicationInfo}>
          ...
          {app.notes && <Text style={styles.notes}>메모: {app.notes}</Text>}
        </View>
      </View>
  ...
  const renderTypeSection = (type, applications) => {
    ...
    const iconName = type.includes("외국인등록")
    ...
    return (
      <View key={type} style={styles.typeSection}>
        ...
        {applications.map((app, index) => (
          <View key={index}>{renderApplicationItem(app)}</View>
        ))}
      </View>
    );
  };
