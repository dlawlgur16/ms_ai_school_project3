...
const SignUpScreen = () => {
  ...
  const [showPassportVerification, setShowPassportVerification] =
    useState(false);
  ...
  const pickImage = async () => {
    try {
      Alert.alert("여권 이미지 업로드", "이미지를 가져올 방법을 선택하세요", [
        {
          text: "카메라로 촬영",
          onPress: () => takePhoto(),
        },
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!data.surname || !data.givenname || !data.passport_number) {
          Alert.alert(
            "알림",
            "일부 여권 정보를 추출하지 못했습니다. 수동으로 입력해주세요."
          );
        }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!data.name_kor || !data.resident_id || !data.visa_type) {
          Alert.alert(
            "알림",
            "일부 외국인 등록증 정보를 추출하지 못했습니다. 수동으로 입력해주세요."
          );
        }
        ...
        Alert.alert(
          "오류",
          data.error || "외국인 등록증 정보 추출에 실패했습니다."
        );
      }
    } catch (error) {
      ...
      if (error.message.includes("Network request failed")) {
        ...
        Alert.alert(
          "오류",
          "외국인 등록증 분석 중 오류가 발생했습니다. 다시 시도해주세요."
        );
      }
    } finally {
      setIsAnalyzingResidence(false);
          ...
          <View style={styles.form}>
            ...
            <View style={styles.passportSection}>
              <Text style={styles.passportLabel}>
                Passport Registration (Optional)
              </Text>
              <TouchableOpacity
              ...
              {passportData && (
                <View style={styles.passportInfoCard}>
                  ...
                  <View style={styles.passportInfoRow}>
                    ...
                    <Text style={styles.passportInfoValue}>
            ...
            <View style={styles.passportSection}>
              <Text style={styles.passportLabel}>
                Residence Card Registration (Optional)
              </Text>
              <TouchableOpacity
                style={styles.passportButton}
              ...
              >
                ...
                />
                <Text style={styles.passportButtonText}>
                  {isAnalyzingResidence
                    ? "Analyzing residence card..."
                    : residenceData
                    ? "Residence Card Registered"
                    : "Upload Residence Card Image"}
                </Text>
              </TouchableOpacity>
              {residenceCardImage && (
                <Image
                ...
                <View style={styles.passportInfoCard}>
                  <View style={styles.passportInfoHeader}>
                    <Ionicons
                      ...
                      style={{ marginRight: 6 }}
                    />
                    <Text style={styles.passportInfoTitle}>
                      Residence Card Info
                    </Text>
                  </View>
                  <View style={styles.passportInfoRow}>
            ...
            <TouchableOpacity
              style={[
                styles.button,
                (!username || !password || !name) && styles.buttonDisabled,
              ]}
