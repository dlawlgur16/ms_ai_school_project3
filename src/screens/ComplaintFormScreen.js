...
const ComplaintFormScreen = () => {
  ...
  const questions = [
    ...
    {
      id: "workplaceType",
      text: "사업장 유형을 선택해주세요. (1: 사업장, 2: 공사현장)",
    },
    ...
    {
      id: "Date of Resignation/Termination",
      text: '퇴사일을 입력해주세요. (YYYY-MM-DD, 재직중이면 "재직중" 입력)',
    },
    {
      id: "Total Amount of Unpaid Wages",
      text: "미지급 임금 총액을 입력해주세요.",
    },
    {
      id: "employmentStatus",
      text: "재직 여부를 선택해주세요. (1: 퇴사, 2: 재직중)",
    },
    {
      id: "Amount of Unpaid Severance Pay",
      text: "미지급 퇴직금을 입력해주세요.",
    },
    {
      id: "Other Unpaid Amounts",
      text: '기타 미지급금을 입력해주세요. (없으면 "없음" 입력)',
    },
    { id: "Job Description", text: "담당업무를 입력해주세요." },
    { id: "Wage Payment Date", text: "임금지급일을 입력해주세요." },
    {
      id: "contractType",
      text: "계약서 작성 여부를 선택해주세요. (1: 서면, 2: 구두)",
    },
    {
      id: "work_detail",
      text: "1. 어떤 일을 하셨나요? (직무와 담당한 작업 내용을 알려주세요)",
    },
    {
      id: "period",
      text: "2. 언제부터 언제까지 일하셨고, 그 중 임금을 받지 못한 기간은 언제인가요?",
    },
    {
      id: "location",
      text: "3. 어느 지역, 어떤 회사(또는 인력사무소)에서 일하셨나요? 정확한 주소를 알려주세요.",
    },
    {
      id: "wage",
      text: "4. 월급은 원래 얼마였고, 체불된 금액은 총 얼마인가요?",
    },
    {
      id: "response",
      text: "5. 임금 체불에 대해 사업주에게 요청해보신 적이 있나요? 어떤 대응이 있었나요?",
    },
    {
      id: "extra_info",
      text: "6. 추가로 제가 알아야 하는 내용을 더 알려주세요.",
    },
  ];
      ...
      try {
        if (languageCode === "ko") {
          ...
          const translatedQuestionTexts = await translateMultipleTexts(
            Object.fromEntries(
              questionTexts.map((text, index) => [index, text])
            ),
            languageCode
          );

          // 선택지 번역
          const translatedOptions = await translateMultipleTexts(
            optionTexts,
            languageCode
          );
  ...
  const handleSubmit = async () => {
    ...
    if (currentQ.id === "processStatus") {
      formData[
        "cReceive Processing Status Notifications " +
          (value.toLowerCase() === "y" ? "yes" : "no")
      ] = "y";
    } else if (currentQ.id === "laborPortal") {
      formData[
        "cReceive Notifications via Labor Portal " +
          (value.toLowerCase() === "y" ? "yes" : "no")
      ] = "y";
    } else if (currentQ.id === "workplaceType") {
      formData[value === "1" ? "Workplace" : "Construction site"] = "y";
    } else if (currentQ.id === "employmentStatus") {
      formData[value === "1" ? "Resigned/terminated" : "Currently employed"] =
        "y";
    ...
    if (isLastDetailQuestion && allDetailsAnswered) {
      ...
      try {
        ...
        const contentResponse = await axios.post(
          `${SERVER_URL}/api/generate_complaint_content`,
          detailData
        );
        ...
        if (contentResponse.data.success) {
          ...
          const evidenceResponse = await axios.post(
            `${SERVER_URL}/api/generate_complaint_evidence`,
            {
          ...
          if (evidenceResponse.data.success) {
            setMessages((prev) => [
              ...prev,
              {
                type: "bot",
                text:
                  "진정 내용이 생성되었습니다. 증거자료 안내를 확인해주세요:\n\n" +
          ...
          const response = await axios.post(
            `${SERVER_URL}/api/generate-complaint-pdf`,
            formData
          );
      ...
      setMessages((prev) => [
        ...prev,
        { type: "bot", text: translatedQuestions[currentQuestion + 1].text },
      ]);
    }
  };
    ...
    try {
      ...
      if (Platform.OS === "android") {
        ...
        if (granted !== PermissionsAndroid.RESULTS.GRANTED) {
          Alert.alert(
            "권한 필요",
            "PDF를 다운로드하기 위해 저장소 접근 권한이 필요합니다."
          );
          return;
        }
      }
      ...
      if (downloadResult.status === 200) {
        if (Platform.OS === "ios") {
          ...
          Alert.alert("다운로드 완료", "PDF가 다운로드 폴더에 저장되었습니다.");
        }
  ...
  const renderInputField = () => {
    if (!translatedQuestions.length || !translatedQuestions[currentQuestion])
      return null;
    const currentQ = translatedQuestions[currentQuestion];
        ...
        <View style={styles.selectionContainer}>
          <TouchableOpacity
            style={[
              ...
              inputValue === "Y" && styles.selectionButtonSelected,
            ]}
          ...
          >
            <Text
              style={[
                ...
                inputValue === "Y" && styles.selectionButtonTextSelected,
              ]}
            >
          ...
          <TouchableOpacity
            style={[
              styles.selectionButton,
              inputValue === "N" && styles.selectionButtonSelected,
            ]}
          ...
          >
            <Text
              style={[
                styles.selectionButtonText,
                inputValue === "N" && styles.selectionButtonTextSelected,
              ]}
            >
      ...
      return (
        <View style={styles.selectionContainer}>
          <TouchableOpacity
            style={[
              styles.selectionButton,
              inputValue === "1" && styles.selectionButtonSelected,
            ]}
          ...
          >
            <Text
              style={[
                styles.selectionButtonText,
                inputValue === "1" && styles.selectionButtonTextSelected,
              ]}
            >
          ...
          <TouchableOpacity
            style={[
              styles.selectionButton,
              inputValue === "2" && styles.selectionButtonSelected,
            ]}
          ...
          >
            <Text
              style={[
                styles.selectionButtonText,
                inputValue === "2" && styles.selectionButtonTextSelected,
              ]}
            >
              {translatedOptions.constructionSite ||
                optionTexts.constructionSite}
            </Text>
          </TouchableOpacity>
        </View>
      );
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.pdfFooter}>
          <TouchableOpacity style={styles.pdfFooterButton} onPress={handleBack}>
          ...
          >
            ...
            <Text
              style={[
                styles.pdfFooterButtonText,
                styles.pdfFooterButtonTextPrimary,
              ]}
            >
              공유하기
            </Text>
