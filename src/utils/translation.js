...
const AZURE_TRANSLATOR_LOCATION = "";
...
export const translateMultipleTexts = async (texts, targetLanguage) => {
  console.log("Translating multiple texts:", {
    textCount:
      typeof texts === "object" ? Object.keys(texts).length : texts.length,
    targetLanguage,
  });

  if (!texts || !targetLanguage) {
    console.error("Missing texts or target language:", {
      texts,
      targetLanguage,
    });
    throw new Error("텍스트와 대상 언어가 필요합니다.");
